...
git_init(){
    git config --global user.name "caoqib"
    git config --global user.email caoqib@yonyou.com
    git remote rm origin
    git remote add origin git@github.com:caoqib/gcr.git
    git pull
    if git branch -a |grep 'origin/master' &> /dev/null ;then
        git checkout master
